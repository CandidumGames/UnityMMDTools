...
{
    /// <summary>
    /// Raw animation curve data produced by <see cref="VMDAnimationClipConverter"/> in place of a Unity
    /// <see cref="AnimationClip"/>. A <see cref="VMDClipData"/> is a flat set of <see cref="AnimationCurve"/> channels
    /// grouped by target path: <see cref="curves"/> holds <c>x</c> consecutive channels per entry in <see cref="paths"/>,
    /// where <c>x</c> is fixed for the kind of data and known by whatever code produces or consumes the instance
    /// (it is not stored here). A channel may be <c>null</c> when it was not written.
    /// </summary>
    /// <remarks>
    /// Why curves instead of an <see cref="AnimationClip"/>: <see cref="AnimationClip.SetCurve"/> only works at runtime
    /// for legacy clips, and Unity Timeline's <c>AnimationTrack</c> rejects legacy clips. To play VMD motion through
    /// Timeline at runtime the demo evaluates these curves directly in a custom playable and writes the transforms
    /// itself. The editor importers reconstruct an <see cref="AnimationClip"/> from this data (where <c>SetCurve</c> is
    /// valid) to keep writing <c>.anim</c> sub-assets.
    ...
    public sealed class VMDClipData
    {
        ...
        /// <summary>Allocates the path and curve arrays for <paramref name="pathCount"/> paths with <paramref name="channelsPerPath"/> channels each.</summary>
    ...
    /// <summary>
    /// Blendshape (morph) weight curves. Parallel arrays: <see cref="VMDClipData.paths"/> holds the renderer path and
    /// <see cref="VMDClipData.curves"/> the weight curve (one channel per path), while <see cref="names"/> holds the
    /// blendshape name for the same index so consumers can reconstruct the <c>blendShape.&lt;name&gt;</c> binding.
    /// </summary>
    ...
    /// <summary>
    /// Full result of converting the body of a VMD animation: bone curves, morph curves, and (non-baked only) IK toggle
    /// curves. The <see cref="baked"/> flag records whether <see cref="bones"/> uses the baked 7-channel quaternion
    /// layout or the non-baked 6-channel euler layout, so consumers pick the matching channel order and property names.
    /// </summary>
    ...
    /// <summary>
    /// Camera curve data for the two-node VMD camera rig. <see cref="VMDClipData.paths"/> holds the rig-relative paths
    /// (the look-at target and its camera child) and <see cref="VMDClipData.curves"/> the channels for each, laid out in
    /// the order documented by <see cref="VMDAnimationClipConverter"/>: target localPosition x/y/z, target localRotation
    /// x/y/z/w, camera child localPosition.z, and camera field of view.
    /// </summary>
